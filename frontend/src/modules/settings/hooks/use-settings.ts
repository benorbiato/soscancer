...
export function useSettings() {
  ...
  const handleImageUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!SETTINGS_CONSTANTS.PROFILE_IMAGE.ACCEPTED_TYPES.includes(file.type)) {
      toast.error('Tipo de arquivo não suportado', SETTINGS_CONSTANTS.MESSAGES.ERROR.INVALID_IMAGE_TYPE)
      return
    }
  ...
  const togglePasswordVisibility = useCallback((field: 'currentPassword' | 'newPassword' | 'confirmPassword') => {
    setState(prev => ({
      ...prev,
      [`show${field.charAt(0).toUpperCase() + field.slice(1)}`]: !prev[`show${field.charAt(0).toUpperCase() + field.slice(1)}` as keyof SettingsState]
    }))
  }, [])

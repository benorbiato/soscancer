...
export function useRegistryForm() {
  ...
  const handleSubmit = useCallback(async (values: RegistryFormData) => {
  ...
  const togglePasswordVisibility = useCallback((field: 'password' | 'confirmPassword') => {
    setState(prev => ({
      ...prev,
      [`show${field.charAt(0).toUpperCase() + field.slice(1)}`]: !prev[`show${field.charAt(0).toUpperCase() + field.slice(1)}` as keyof RegistryState]
    }))
  }, [])
